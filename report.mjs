...
export function buildReport(input,{date=new Date()}={}){
 ...
 const sourceLinks=[['厚生労働省・DC制度','https://www.mhlw.go.jp/stf/nenkin_shikumi_015.html'],['国税庁・所得税','https://www.nta.go.jp/publication/pamph/koho/kurashi/html/02_1.htm'],['協会けんぽ・料率','https://www.kyoukaikenpo.or.jp/about/business/insurance_rate/rate_prefectures/r08/'],['日本年金機構・報酬比例部分','https://www.nenkin.go.jp/service/yougo/hagyo/hoshuhirei.html'],['厚生労働省・令和6年簡易生命表','https://www.mhlw.go.jp/toukei/saikin/hw/life/life24/dl/life24-02.pdf'],['厚生労働省・給付上限','https://www.mhlw.go.jp/content/001728499.pdf']];
...
<div class="hero"><div class="hero-metric"><span>${headline}／年</span><strong>${yen(Math.abs(r.savings))}</strong><small>月平均 ${yen(Math.abs(r.savings)/12)}</small></div><div class="hero-metric hero-total"><span>拠出満了までの軽減総額</span><strong>${yen(Math.abs(r.totalSavings))}</strong><small>${x.age}歳から${x.endAge}歳・${r.months/12}年間の単純累計</small></div><div class="hero-details"><p>年間内訳</p><p>所得税等 <b>${yen(r.tax)}</b></p><p>住民税所得割 <b>${yen(r.resident)}</b></p><p>社会保険料 <b>${yen(r.social)}</b></p></div></div>
...
<p class="caption">所得税は年末調整等の後、住民税は翌年度分の軽減を反映した比較です。軽減総額は現在の給与・税率・保険料率と年間軽減額が拠出満了まで変わらない前提です。実際の毎月の給与明細ではありません。賞与・賞与の保険料も12か月に均しており、端数処理で内訳と合計が一致しない場合があります。</p>
...
<div class="pension-lifetime"><span>65歳から平均余命までの累計減少額（参考）</span><p>男性の場合 <b>${yen(r.pensionLifetimeLossMale)}</b></p><p>女性の場合 <b>${yen(r.pensionLifetimeLossFemale)}</b></p></div>
<p class="caption">${x.age}歳から${x.endAge}歳まで（${x.endAge-x.age}年間・${r.months}か月）、同じ条件で拠出する前提。累計減少額は厚生労働省「令和6年簡易生命表」の65歳時平均余命（男性19.47年・女性24.38年）を使い、年額の減少目安が同額で続くとした単純累計です。受給開始年齢・改定率・在職老齢年金・死亡年齢は未反映です。</p>
<h2>他の手当金・給付金への影響</h2>
...
/* Color and total-value hierarchy for the A4 report. */
.rate-notice-print{border-left-color:#148b91;background:linear-gradient(90deg,#eaf8fa,#f2f6fb)}
header{border-bottom-color:#148b91}.overline{color:#167985}h1{color:#173e60}.conditions{background:linear-gradient(90deg,#edf7fb,#f4f7fb);border-color:#cfe0e8}
.hero{background:linear-gradient(135deg,#173e60,#126f78);grid-template-columns:1fr 1fr .88fr;padding:3mm 4mm;gap:4mm}.hero span{font-size:9pt}.hero strong{font-size:23pt;white-space:nowrap}.hero small{font-size:7.5pt}.hero-total{background:#ffffff12;border-radius:2mm;padding:1mm 2mm}.hero-total strong{color:#fff2aa}.hero-details{border-left-color:#8ac8cd;padding-left:4mm}.hero-details p{margin:.6mm 0;font-size:8pt}.hero-details p:first-child{color:#cde8ea;font-weight:700}
.takehome{background:#e4f3f5}.fund-box{border-color:#9fcbd0;background:linear-gradient(90deg,#f5fbff,#f2fbf8)}.risks{gap:.25mm}.risk-intro{border-left-color:#d17a32;background:#fff3e8}.pension-box{background:#fffaf4}
.pension-lifetime{display:grid;grid-template-columns:1.4fr .8fr .8fr;align-items:center;gap:3mm;border:1px solid #edd4b9;background:#fffdf9;padding:.6mm 3mm;font-size:8pt}.pension-lifetime>span{color:#6c4c30}.pension-lifetime p{margin:0;display:flex;justify-content:space-between;gap:2mm}.pension-lifetime b{color:#89511f;white-space:nowrap;font-size:9pt}.benefit-table thead th{background:#e8f5f5}
`;
